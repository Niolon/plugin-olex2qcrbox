...
class GUIController:
    """Facade for all Olex2 GUI interactions.
    
    Centralizes writing to GUI elements and reading color schemes.
    """
    
    def __init__(self, OV, olx):
        """Initialize GUI controller with Olex2 function objects.
    ...
    def get_olex2_colors(self) -> dict:
        ...
        try:
            return {
                'bg_color': self.olx.GetVar('HtmlBgColour', '#222222'),
                'font_color': self.olx.GetVar('HtmlFontColour', '#ffffff'),
                'table_bg': self.olx.GetVar('HtmlTableBgColour', '#222222'),
                'highlight': self.olx.GetVar('HtmlHighlightColour', '#ff8888'),
                'link_color': self.olx.GetVar('HtmlLinkColour', '#ababff'),
                'font_name': self.olx.GetVar('HtmlFontName', 'Bahnschrift'),
                'error_color': self._get_color_hex(self.OV.GetParam('gui.red', '#ff6666')),
                'secondary_color': self._get_color_hex(self.OV.GetParam('gui.grey', '#aaaaaa')),
            }
    ...
    def update_run_button(self, text: str, color: str, enabled: bool):
    ...
    def update_parameter_panel(self, html_content: str):
        ...
        """
        try:
            self.OV.write_to_olex("qcb-parameters.htm", html_content)
            print(f"[GUI] Parameter panel updated")
        except Exception as e:
            print(f"[GUI] Failed to update parameter panel: {e}")
    
    def update_help_file(self, qcrbox_available: bool, applications: list, 
        ...
        try:
            ...
            self.olx.SetVar('qcrbox_command_help', help_html)
    ...
    def open_file_in_olex2(self, file_path: str):
